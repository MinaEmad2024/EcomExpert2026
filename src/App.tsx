...
function Builder({ desktopAlt }: { desktopAlt: boolean }) {
  ...
  return (
    <main className={`flex min-w-0 flex-col ${desktopAlt ? 'w-full gap-0' : 'w-[768px] gap-step max-lg:w-full'}`}>
  ...
  return (
    <BundleProvider>
      <div className={desktopAlt ? 'desktop-alt mx-auto max-w-[1230px] py-10' : 'mx-auto max-w-[1191px] px-6 py-10 max-lg:px-0 max-lg:pt-8'}>
        ...
        <div className={desktopAlt ? 'flex flex-col gap-8' : 'flex items-start gap-6 max-lg:flex-col max-lg:items-stretch'}>
          <Builder desktopAlt={desktopAlt} />
          {desktopAlt ? (
